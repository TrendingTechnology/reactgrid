...
export function selectRange(state, range, incremental) {
    return __assign(__assign({}, state), { selectionMode: 'range', selectedRanges: (incremental && state.selectionMode === 'range' ? state.selectedRanges : []).concat([range]), selectedIndexes: [], selectedIds: [], activeSelectedRangeIdx: incremental && state.selectionMode === 'range' ? state.selectedRanges.length : 0 });
}
export function updateActiveSelectedRange(state, range) {
    var _a;
    return __assign(__assign({}, state), { selectionMode: 'range', selectedRanges: Object.assign([], state.selectedRanges, (_a = {}, _a[state.activeSelectedRangeIdx] = range, _a)), selectedIndexes: [], selectedIds: [] });
}
export function selectOneColumn(state, col, incremental) {
    return __assign(__assign({}, state), { selectionMode: 'column', selectedIndexes: (incremental && state.selectionMode === 'column' ? state.selectedIndexes : []).concat(col.idx), selectedIds: (incremental && state.selectionMode === 'column' ? state.selectedIds : []).concat(col.columnId) });
}
export function unSelectOneColumn(state, col) {
    ...
    return __assign(__assign({}, state), { selectionMode: 'column', selectedIndexes: updatedIndexes, selectedIds: updatedIds });
}
...
export function selectOneRow(state, row, incremental) {
    return __assign(__assign({}, state), { selectionMode: 'row', selectedIndexes: (incremental && state.selectionMode === 'row' ? state.selectedIndexes : []).concat(row.idx), selectedIds: (incremental && state.selectionMode === 'row' ? state.selectedIds : []).concat(row.rowId) });
}
export function unSelectOneRow(state, row) {
    ...
    return __assign(__assign({}, state), { selectionMode: 'row', selectedIndexes: updatedIndexes, selectedIds: updatedIds });
}
