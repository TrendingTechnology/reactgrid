...
export function selectOneColumn(state, col, incremental) {
    return __assign({}, state, { selectionMode: 'column', selectedIndexes: (incremental && state.selectionMode === 'column' ? state.selectedIndexes : []).concat(col.idx), selectedIds: (incremental && state.selectionMode === 'column' ? state.selectedIds : []).concat(col.id) });
}
export function unSelectOneColumn(state, col) {
    ...
    var updatedIds = state.selectedIds.filter(function (id) { return id !== col.id; });
...
export function selectMultipleColumns(state, firstCol, lastCol, incremental) {
    ...
    var range = state.cellMatrix.getRange(new Location(firstRow, firstCol), new Location(lastRow, lastCol));
    return __assign({}, state, { selectionMode: 'column', selectedIndexes: incremental ? state.selectedIndexes.concat(range.cols.map(function (col) { return col.idx; })) : range.cols.map(function (col) { return col.idx; }), selectedIds: incremental ? state.selectedIds.concat(range.cols.map(function (col) { return col.id; })) : range.cols.map(function (col) { return col.id; }) });
}
export function selectOneRow(state, row, incremental) {
    return __assign({}, state, { selectionMode: 'row', selectedIndexes: (incremental && state.selectionMode === 'row' ? state.selectedIndexes : []).concat(row.idx), selectedIds: (incremental && state.selectionMode === 'row' ? state.selectedIds : []).concat(row.id) });
}
export function unSelectOneRow(state, row) {
    var updatedIndexes = state.selectedIndexes.filter(function (idx) { return idx !== row.idx; });
    var updatedIds = state.selectedIds.filter(function (id) { return id !== row.id; });
...
export function selectMultipleRows(state, firstRow, lastRow, incremental) {
    ...
    var range = state.cellMatrix.getRange(new Location(firstRow, firstCol), new Location(lastRow, lastCol));
    return __assign({}, state, { selectionMode: 'row', selectedIndexes: incremental ? state.selectedIndexes.concat(range.rows.map(function (row) { return row.idx; })) : range.rows.map(function (row) { return row.idx; }), selectedIds: incremental ? state.selectedIds.concat(range.rows.map(function (row) { return row.id; })) : range.rows.map(function (row) { return row.id; }) });
}
