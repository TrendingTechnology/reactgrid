...
function handleKeyDownInternal(state: State, event: KeyboardEvent): State {
    ...
    if (event.ctrlKey && event.shiftKey) {
        ...
        switch (event.keyCode) {
            ...
            case keyCodes.ENTER:
                return isSingleCellSelected ? { ...moveFocusDown(state), currentlyEditedCell: undefined } : moveFocusInsideSelectedRange(state, 'down', asr, location);
            case keyCodes.ESC:
...
function moveFocusDown(state: State): State {
    return state.focusedLocation && state.focusedLocation.row.idx < state.cellMatrix.last.row.idx ? focusCell(state.focusedLocation.column.idx, state.focusedLocation.row.idx + 1, state) : state;
}

// TODO this should be rewritten
function moveFocusPageUp(state: State): State {
    if (!state.focusedLocation) return state;
    const rowsOnScreen = state.cellMatrix.rows.filter((r: GridRow) => r.top < state.viewportElement.clientHeight);
